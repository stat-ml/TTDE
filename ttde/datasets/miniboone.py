...
def load_miniboone_dataset(path: Path, to_jax: bool = True):
    data = MINIBOONE(path / 'miniboone' / 'data.npy')

    data_train = TensorDatasetX(data.trn.x)
    data_val = TensorDatasetX(data.val.x)
